...
fn part1(image: &[u8]) -> usize {
    let layer = image.chunks(HEIGHT*WIDTH)
...
fn part2(image: &[u8]) -> String {
    ...
    let final_image: Vec<_> = (0..(HEIGHT*WIDTH)).map(|pos| pixel(&layers, pos)).collect();
    render(&final_image)
}

fn render(image: &[u8]) -> String {
    let text: Vec<_> = image.iter()
        .map(|&x| if x == 1 {'x' as u8} else {' ' as u8})
        .collect();
    let rows: Vec<_> = text.chunks(WIDTH)
